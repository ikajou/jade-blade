...
function expectStream(t, options){
  options = options || {};
  var ext = options.client ? '.js' : '.blade.php';
...
test('should compile my jade files into Blade', function(t){
  gulp.src(filename)
    .pipe(task())
    .pipe(expectStream(t));
});

test('should compile my jade files into Blade with locals passed in', function(t){
  gulp.src(filename)
    .pipe(task({
      locals: {
        title: 'Yellow Curled'
...
test('should compile my jade files into Blade with data passed in', function(t){
  gulp.src(filename)
    .pipe(task({
      data: {
        title: 'Yellow Curled'
...
test('should compile my jade files into Blade with data property', function(t){
  gulp.src(filename)
    .pipe(setData())
    .pipe(task())
    .pipe(expectStream(t, {
